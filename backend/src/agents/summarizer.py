...
class Summarizer:
    ...
    def __init__(
        self,
        summarizer_factory: Callable[[], ToolAwareSimpleAgent],  # summarizer_factory是一个函数对象, 此句表示一个无参数函数, 返回 ToolAwareSimpleAgent 实例
    ...
    def stream_task_summary(
        ...
        # 用于流式输出时实时过滤思考标记, 只输出可见内容给用户
        # flush_visible()是边生成边过滤, 实时处理thinking标记, strip_thinking_tokens是事后处理
        # ✅️
