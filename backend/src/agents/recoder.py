...
class ToolAwareSimpleAgent(SimpleAgent):
    ...
    def _execute_tool_call(self, tool_name: str, parameters: str) -> str:  # type: ignore[override]
        ...
        try:
            ...
            formatted_result = f"✅️ 工具执行成功 🔧 工具 {tool_name} 执行结果：\n{result}\n"
        ...
        if self._tool_call_listener:
            try:
                ...
                formatted_result += "✅️ 监听器通知成功"
            except Exception:  # pragma: no cover - 防御性兜底
                logger.exception("Tool call listener failed")
                formatted_result += "❌ 监听器通知失败"
    ...
    # ✅️
    # 用于清理字符串, 移除多余的引号和不完整的括号(AI返回的参数可能格式不规范)
    # 主要处理这三种情况(1) "hello 或 'hello 或 hello" 或 hello' (2) "hello" 或 'hello' (3) [1, 2, 3 或 1, 2, 3]
    @staticmethod
    def _normalize_string(value: str) -> str:
        ...
        # 即使情况很少见也建议加上这一句, 做到万无一失
        if trimmed and trimmed[-1] in {']', ')'} and trimmed[0] not in {'[', '('}:
            start = '[' if trimmed[-1] == ']' else '('
            trimmed = f"{start}{trimmed}"
    ...
    def stream_run(self, input_text: str, max_tool_iterations: int = 3, **kwargs: Any) -> Iterator[str]:  # type: ignore[override]
        ...
        while current_iteration < max_tool_iterations:
            ...
            def process_residual(final_pass: bool = False) -> Iterator[str]:
                ...
                while True:
                    ...
                    if start == -1:
                        # bracket_pos = residual.rfind('[')
                        # if bracket_pos == -1 or final_pass:
                        #     yield residual
                        #     residual = ""
                        # else:
                        #     yield residual[:bracket_pos]
                        #     residual = residual[bracket_pos:]
                        # (1)有bug (2)这里会导致用户看到的内容有延迟, 建议只保留[之后的内容
                        # chunk1: 好的! chunk2: [TOOL_CALL, 处理后segment = 好的![, [TOOL_CALL被截断
                    ...
                    # 得到类似['[TOOL_CALL:get_weather:{"city": "北京"}]']
            ...
            # 将['[TOOL_CALL:get_weather:{"city": "北京"}]']解析为类似[{"tool_name": "get_weather", "parameters": '{"city": "北京"}', "original": "..."}]
        ...
        if current_iteration >= max_tool_iterations and not final_response_text:
            ...
            yield '❗️ 已达到最大调用次数, 检测到循环调用工具, 返回默认回复：' + fallback_response
