...
实现了工具调用事件的追踪和同步机制
Agent 调用工具
1、ToolCallTracker.record() 记录事件
2、ToolCallTracker.drain() 提取事件
3、drain() 同步 note_id → 更新 TodoItem.note_id / note_path
4、返回事件列表 → 用于 SSE 流式推送或日志
"""
...
class ToolCallTracker:
    ...
    def drain(self, state: SummaryState, *, step: Optional[int] = None) -> list[dict[str, Any]]:
        ...
        with self._lock:
            if self._cursor >= len(self._events):
                return []
            new_events = self._events[self._cursor :]  # 从游标位置切片，获取未消费的事件
            self._cursor = len(self._events)  # 更新游标到最新位置
    ...
    def set_event_sink(self, sink: Optional[Callable[[dict[str, Any]], None]]) -> None:
        """注册一个回调函数，用于接收工具事件的实时通知"""

        self._event_sink = sink
    
    # ✅️
    # 将内部事件对象转换为前端可用的 SSE payload 格式
        ...
        for task in tasks:
            ...
            if task.note_id != note_id:
                ...
                if self._notes_workspace:
                    task.note_path = str(Path(self._notes_workspace) / f"{note_id}.md")  # 任务更新, 创建新的笔记
